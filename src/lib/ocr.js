 ...
 */
export default function ocr(imgPath, options, cb) {

  let requestOptions, requestFormData = {};

  if (_.isFunction(options)) [cb, options] = [options, {}];

  let _opts = _.assign({}, defaultOCROptions, _.pick(options, _.keys(defaultOCROptions)));
  if (!_opts.image) _opts.image = fs.createReadStream(imgPath);

  _.each(_opts, (val, key) => {
  ...
  let handler = (err, result) => {
    ...
    else ylog.ok.no.wrap.ln.log(result).ln();
    if (cb) cb(err, result);
  };
  ...
  request(requestOptions, (err, res, body) => {
    ...
    if (ret.errNum === '0') {
      handler(null, _.map(ret.retData, (it) => { return it.word; }).join(''));
    } else {
      handler(ret);
    }
  });
